 ...
 */
public class GantaNewsActivity extends Activity implements View.OnClickListener {
    ...
    ImageView ivQuanmao;
    @Bind(R.id.iv_gantaImage)
    ImageView ivGantaImage;
    @Bind(R.id.et_beizhu)
    EditText etBeizhu;
    ...
    public static GantaNewsActivity GantaNewsActivity;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        parentView = getLayoutInflater().inflate(R.layout.gantanewsactivity_layout, null);
        setContentView(parentView);
        GantaNewsActivity = this;
        ButterKnife.bind(this);
        initView();
        pictureDatabase = new PictureDatabase(this);
    }

    private void initView() {//输入框只能输入数字和小数点
        ...
        titleTitle.setText("新增杆塔信息");
        quit.setVisibility(View.GONE);
        btnFinish.setOnClickListener(this);
        noScrollgridview.setOnClickListener(this);
        ivGantaImage.setOnClickListener(this);
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_finish:
                if (click() == true) {
                    offlineSave();
                    if(bmp!=null){
                        pictureDatabase.saveBianYaQiPhoto(bmp);
                    }
                    if(bmp1!=null) {
                        pictureDatabase.saveBianYaQiPhoto(bmp1);
                    }
                    if(bmp2!=null) {
                        pictureDatabase.saveGanTaPhoto(bmp2);
                    }
                    finish();
                    sendBroadcast(new Intent(AddSheBeiMapActivity.FINISH_ACTIVITY));
                    Toast.makeText(GantaNewsActivity.this, "新增杆塔完成", Toast.LENGTH_SHORT).show();
                }
                break;
    ...
    private Boolean click() {
        ...
        if (Fangwei.equals("")) {
            Toast.makeText(GantaNewsActivity.this, "请输入设备名称", Toast.LENGTH_SHORT).show();
            return false;
        }
...
//     */
//    private void offlineSave() {
//        GanTaBean beans = new GanTaBean();
//        beans.setUuid(System.currentTimeMillis() + "");
//        //编号
//        beans.setName(etName.getText().toString());
//        //材质
//        beans.setJingdu(tvJingdu.getText().toString());
...
//        beans.setXinznegshijian(tvXinzengshijain.getText().toString());
//        try {
//            x.getDb(daoConfig).save(beans);
//            //x.getDb(daoConfig).delete(SNBean.class, WhereBuilder.b("sn", "=", snBean.getSn()));//删除编号
//        } catch (DbException e) {
//            e.printStackTrace();
//        }
//    }
...
//                .singleSelect()
        ...
        ImageSelector.open(GantaNewsActivity, imageConfig);   // 开启图片选择器
    }
                ...
                if (requestCode == REQUEST_CODE && resultCode == RESULT_OK && data != null) {
                    ...
                    if (pathList != null && pathList.size() > 0) {
                        if (cPicState.equals("1")) {
                            ...
                            ImageLoader.getInstance().displayImage("file://" + img2Path, ivGantaImage);
                            bmp1 = BitmapFactory.decodeByteArray(ImageUtil.decodeBitmap(img2Path), 0,ImageUtil.decodeBitmap(img2Path).length);
                        } else if (cPicState.equals("3")) {
                            img3Path = pathList.get(0);
                            ImageLoader.getInstance().displayImage("file://" + img3Path, ivQuanmao);
                            bmp2 = BitmapFactory.decodeByteArray(ImageUtil.decodeBitmap(img3Path), 0,ImageUtil.decodeBitmap(img3Path).length);
                            ...
                            if (cPicState.equals("1")) {
                                ...
                                img2Path = "";
                                ivGantaImage.setImageResource(R.drawable.picbtn);
                                ivGantaImage.setScaleType(ImageView.ScaleType.FIT_XY);
                            } else if (cPicState.equals("3")) {
                                img3Path = "";

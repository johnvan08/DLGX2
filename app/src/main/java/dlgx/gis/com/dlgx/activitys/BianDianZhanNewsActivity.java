 ...
 */
public class BianDianZhanNewsActivity extends Activity implements View.OnClickListener {
    ...
    public static BianDianZhanNewsActivity BianDianZhanNewsActivity;
    private PictureDatabase pictureDatabase;
    private Bitmap bmp,bmp1;
    private String equipmentType;
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.biandianzhannewsactivity_layout);
        BianDianZhanNewsActivity = this;
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_finish:
                if (click() == true) {
                    offlineSave();
                    if(bmp!=null){
                        pictureDatabase.saveBianDainZhanPhoto(bmp);
                    }
                    if(bmp1!=null) {
                        pictureDatabase.saveBianDainZhanPhoto(bmp1);
                    }
                    finish();
                    sendBroadcast(new Intent(AddSheBeiMapActivity.FINISH_ACTIVITY));
                    Toast.makeText(BianDianZhanNewsActivity.this, "新增变电站", Toast.LENGTH_SHORT).show();
                }
                break;
            default:
                break;
    ...
    private Boolean click() {
        ...
        if (Fangwei.equals("")) {
            Toast.makeText(BianDianZhanNewsActivity.this, "请输入设备名称", Toast.LENGTH_SHORT).show();
            return false;
        }
...
//                .singleSelect()
        ...
        ImageSelector.open(BianDianZhanNewsActivity, imageConfig);   // 开启图片选择器
    }
